...
int idamax(int n, double *dx, int dx_off, int incx)
{
	...
	else if (incx != 1)
	{
		...
		ix = 1 + incx;
#pragma omp parallel for num_threads(4) reduction(+:ix)
		for (int i = 1; i < n; ++i)
		{
#pragma omp critical
...
#pragma omp parallel  for num_threads(4) reduction(+:ix,iy)
			for (int i = 0; i < n; ++i)
			{
#pragma omp atomic
...
#pragma omp parallel for num_threads(4) reduction(+:ix,iy,temp)
			for (int i = 0; i < n; ++i)
			{
...
#pragma omp parallel for num_threads(4) reduction(+:temp)
			for (int i = 0; i < n; ++i) {
